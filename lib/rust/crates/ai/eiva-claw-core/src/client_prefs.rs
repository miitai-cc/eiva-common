...
//! at `~/.eiva/client.json` and is intentionally simple JSON so
...
/// Shared client preferences persisted in `~/.eiva/client.json`.
...
fn prefs_path() -> Option<PathBuf> {
    Some(dirs::home_dir()?.join(".eiva").join("client.json"))
}

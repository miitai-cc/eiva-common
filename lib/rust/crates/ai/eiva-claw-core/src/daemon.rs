...
//!
//! The `gateway start` command spawns the `eiva-gateway` binary as a
...
/// Spawn the `eiva-gateway` binary as a background process.
///
...
/// Run the `eiva-gateway` binary in the foreground.
///
...
/// Find a sibling Eiva binary by name.  Checks:
/// 1. Next to the current executable (same directory).
/// 2. On `$PATH` via the `which` crate (cross-platform).
///
/// `name` is the base name without an extension (e.g. `"eiva-gateway"`,
/// `"eiva-tui"`); `.exe` is appended automatically on Windows.

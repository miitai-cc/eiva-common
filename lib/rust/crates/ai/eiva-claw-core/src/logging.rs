//! Structured logging configuration for Eiva.
//!
...
//!
//! - `RUSTYCLAW_LOG` or `RUST_LOG`: Set log level (e.g., `debug`, `eiva=debug,hyper=warn`)
//! - `RUSTYCLAW_LOG_FORMAT`: Set output format (`pretty`, `compact`, `json`)
//!
//! ## Examples
//!
//! ```bash
//! # Debug logging for Eiva, warn for everything else
//! RUSTYCLAW_LOG=eiva=debug,warn eiva gateway run
//!
//! # JSON output for production
//! RUSTYCLAW_LOG_FORMAT=json eiva gateway run
//! ```
...
pub struct LogConfig {
    /// Log filter directive (e.g., "debug", "eiva=debug,hyper=warn")
    pub filter: String,
    /// Output format
...
impl Default for LogConfig {
    fn default() -> Self {
        Self {
            filter: "eiva=info,warn".to_string(),
...
impl LogConfig {
    ...
    pub fn from_env() -> Self {
        let filter = std::env::var("RUSTYCLAW_LOG")
            ...
            .unwrap_or_else(|_| "eiva=info,warn".to_string());
    ...
    pub fn debug() -> Self {
        Self {
            filter: "eiva=debug,info".to_string(),
            with_file: true,
            ..Default::default()
        }
    }
...
///
/// ```rust,ignore
/// use eiva::logging::{init, LogConfig};
///
...
pub fn init(config: LogConfig) {
    let env_filter = EnvFilter::try_new(&config.filter)
        .unwrap_or_else(|_| EnvFilter::new("eiva=info,warn"));
...
mod tests {
    ...
    fn test_config_from_env() {
        ...
        let config = LogConfig::from_env();
        assert_eq!(config.filter, "eiva=info,warn");

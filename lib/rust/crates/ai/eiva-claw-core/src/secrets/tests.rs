...
fn temp_dir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("eiva_test_{}_{}", std::process::id(), id));
...
fn test_generate_ssh_key() {
    ...
    assert!(pubkey.contains("eiva@agent"));
    ...
    let (meta, val) = m.get_credential("eiva_agent", &ctx).unwrap().unwrap();
    ...
    m.delete_credential("eiva_agent").unwrap();

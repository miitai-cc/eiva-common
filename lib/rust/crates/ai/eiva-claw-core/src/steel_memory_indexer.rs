//! Bridge that lets `eiva-claw-memory-tree` use `steel-memory` as its secondary
//! (semantic) index.
//!
//! `eiva-claw-memory-tree` defines the [`Indexer`](eiva_claw_memory_tree::Indexer) trait but
...
use eiva_claw_memory_tree::{Indexer, MemoryTreeError, Result as MtResult, SemanticHit, StoredChunk};
use std::sync::Arc;
